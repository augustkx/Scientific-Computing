...
#for Ly in [1,1.5,2]:# spectrum of eigenfrequencies with different size L.
    Lx=2*Ly#rectangle
    s+=1
    dy=dx
...
#    dy=Ly/(Iy-1)
    ...
    plt.savefig('vector_rectangle.png')
    plt.clf()
...
#    plt.plot([(-i)**0.5/(2*math.pi) for i in eigenvalue],color[s],label='L='+str(Ly)+', dy=dx='+str(dy))
#plt.title("the spectrum of eigenfrequencies_rectangle")
#plt.legend(loc='upper right')  
#plt.savefig('eigenfrequencies_rectangle3.png')
